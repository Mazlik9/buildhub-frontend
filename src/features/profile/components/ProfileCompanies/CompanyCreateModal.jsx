...
import { companyService } from '@/api/services'; // ← подключаем сервис
...
const companySchema = z.object({
  ...
  inn: z.string().length(10, 'ИНН должен быть ровно 10 цифр').regex(/^\d{10}$/, 'ИНН должен состоять только из цифр'),
  address: z.string().min(5, 'Введите адрес'),
  description: z.string().min(200, 'Описание минимум 200 символов').max(500, 'Максимум 500 символов'),
  logo: z.any().optional(),
});
...
export default function CompanyCreateModal({ isOpen, onClose, onSuccess }) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // Если есть логотип — отправляем как multipart/form-data
      if (logoFile) {
        const formData = new FormData();
        Object.keys(payload).forEach(key => formData.append(key, payload[key]));
        formData.append('logo', logoFile);

        await companyService.create(formData); // ← если бэк ожидает FormData
      } else {
        await companyService.create(payload); // ← обычный JSON
      }
    ...
    >
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="h-full">
          <div className="flex flex-col justify-start items-center w-[1007px] h-[931px] absolute left-24 top-[34px] gap-5">
            ...
            <div className="flex flex-col justify-center items-center self-stretch flex-grow-0 flex-shrink-0 gap-[60px]">
              ...
              <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 w-[980px] gap-10">
                ...
                <div className="flex flex-col justify-start items-start flex-grow gap-[30px]">
                  {/* Название */}
                  <div className="flex flex-col justify-start items-start self-stretch flex-grow-0 flex-shrink-0 gap-[3px]">
                    <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 h-14 gap-3 p-4 rounded-lg bg-white w-full">
                      <input
                        {...register('name')}
                  ...
                  {/* Телефон + ИНН */}
                  <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 gap-[38px] w-full">
                    <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 w-[331px] gap-[3px]">
                      <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 h-14 gap-3 p-4 rounded-lg bg-white w-full">
                        <input
                          {...register('phone')}
                          placeholder="Телефон"
                  ...
                  {/* Адрес */}
                  <div className="flex flex-col justify-start items-start self-stretch flex-grow-0 flex-shrink-0 gap-[3px] w-full">
                    <div className="flex justify-start items-center self-stretch flex-grow-0 flex-shrink-0 h-14 gap-3 p-4 rounded-lg bg-white w-full">
                      <input
                        {...register('address')}
              ...
              <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 w-[1000px] relative gap-[25px]">
                ...
                <p className="flex-grow-0 flex-shrink-0 w-[979px] text-sm text-left text-black">
                  Коротко опишите, чем занимается ваша компания, для кого вы работаете и в чем ваша уникальность. Этот текст может быть использован в вашем профиле на платформе.
                </p>
                ...
                >
                  <div className="flex justify-start items-start self-stretch flex-grow gap-3 p-4 rounded-lg bg-white h-full">
                    <textarea
                      {...register('description')}
                      placeholder="О компании (200-500 символов)"
                      className="flex-grow bg-transparent outline-none text-base text-[#484848] placeholder:text-[#484848]/60 resize-none h-full"
                    />
                  </div>
            ...
            <div className="flex justify-center items-center flex-grow-0 flex-shrink-0 w-[660px] gap-10">
              <button
                type="button"
                onClick={onClose}
                className="flex justify-center items-center flex-grow-0 flex-shrink-0 w-[304.3px] relative gap-4 px-6 py-4 rounded-2xl border-2 border-[#fca311]"
              >
              ...
              <button
                ...
                className="flex justify-center items-center flex-grow-0 flex-shrink-0 w-[304.3px] relative gap-4 px-6 py-4 rounded-2xl text-base font-black text-white"
                style={{
          ...
          <button
            onClick={onClose}
            className="absolute right-6 top-6 w-14 h-14 flex items-center justify-center bg-white/80 rounded-full shadow hover:bg-gray-100 transition"
          >
